// Edit from https://github.com/autowarefoundation/autoware.universe/tree/main/vehicle/accel_brake_map_calibrator/accel_brake_map_calibrator
// 
// Copyright 2022 Guanghang Cai
// 
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
// 
//     http://www.apache.org/licenses/LICENSE-2.0
// 
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "accel_brake_map_calibrator/accel_brake_map_calibrator_node.h"

#include "std_msgs/Bool.h"
#include "std_msgs/Float32MultiArray.h"
#include "std_msgs/MultiArrayDimension.h"
#include "std_msgs/String.h"
#include "visualization_msgs/MarkerArray.h"
#include "tf/transform_datatypes.h"

#include <limits>
#include <algorithm>
#include <cmath>

/**
 * @brief Construct a new Accel Brake Map Calibrator:: Accel Brake Map Calibrator object
 * 
 */
AccelBrakeMapCalibrator::AccelBrakeMapCalibrator() : nh_(""), pnh_("~")
{
  // init node
  transform_listener_ = std::make_shared<tf2_ros::TransformListener>(tf_buffer_);
  // get parameters
  update_hz_ = pnh_.param<double>("update_hz", 20.0);
  auto_update_ = pnh_.param<bool>("auto_update", false);
  covariance_ = pnh_.param<double>("initial_covariance", 0.05);
  velocity_min_threshold_ = pnh_.param<double>("velocity_min_threshold", 0.1);
  velocity_diff_threshold_ = pnh_.param<double>("velocity_diff_threshold", 0.556);
  pedal_diff_threshold_ = pnh_.param<double>("pedal_diff_threshold", 0.03);
  max_steer_threshold_ = pnh_.param<double>("max_steer_thrshold", 0.2);
  max_pitch_threshold_ = pnh_.param<double>("max_pitch_threshold", 0.02);
  max_jerk_threshold_ = pnh_.param<double>("max_jerk_threshold", 1.0);
  pedal_velocity_thresh_ = pnh_.param<double>("pedal_velocity_threshold", 0.15);
  map_update_gain_ = pnh_.param<double>("map_update_gain", 0.02);
  max_steer_ = pnh_.param<double>("max_steer", 13.5);
  max_accel_ = pnh_.param<double>("max_accel", 5.0);
  min_accel_ = pnh_.param<double>("min_accel", -5.0);
  pedal_to_accel_delay_ = pnh_.param<double>("pedal_to_accel_delay", 0.3);
  max_data_count_ = pnh_.param<int>("max_data_count", 200);
  pedal_accel_graph_output_ = pnh_.param<bool>("pedal_accel_graph_output", false);
  progress_file_output_ = pnh_.param<bool>("progress_file_output", false);
  const auto get_pitch_method_str =
    pnh_.param<std::string>("get_pitch_method", std::string("imu"));
  if (get_pitch_method_str.compare("tf") == 0) {
    get_pitch_method_ = GET_PITCH_METHOD::TF;
  } else if (get_pitch_method_str.compare("imu") == 0) {
    get_pitch_method_ = GET_PITCH_METHOD::IMU;
  } else if (get_pitch_method_str.compare("none") == 0) {
    get_pitch_method_ = GET_PITCH_METHOD::NONE;
  } else {
    ROS_ERROR_STREAM("[accel_brake_map_calibrator] " << "get_pitch_method is wrong. (available : tf, imu, none.");
    return;
  }
  update_suggest_thresh_ = pnh_.param<double>("update_suggest_thresh", 0.7);
  csv_calibrated_map_dir_ = pnh_.param<std::string>("csv_calibrated_map_dir", std::string(""));
  output_accel_file_ = csv_calibrated_map_dir_ + "accel_map.csv";
  output_brake_file_ = csv_calibrated_map_dir_ + "brake_map.csv";
  const std::string update_method_str = pnh_.param<std::string>("update_method", std::string("update_offset_each_cell"));
  if (update_method_str.compare("update_offset_each_cell") == 0) {
    update_method_ = UPDATE_METHOD::UPDATE_OFFSET_EACH_CELL;
  } else if (update_method_str.compare("update_offset_total") == 0) {
    update_method_ = UPDATE_METHOD::UPDATE_OFFSET_TOTAL;
  } else {
    ROS_ERROR_STREAM("[accel_brake_map_calibrator] " << "update_method is wrong. (available : update_offset_total, update_offset_each_cell.");
    return;
  }
  csv_default_map_dir_ = pnh_.param<std::string>("csv_default_map_dir", std::string(""));
  const std::string csv_path_accel_map = csv_default_map_dir_ + "/accel_map.csv";
  const std::string csv_path_brake_map = csv_default_map_dir_ + "/brake_map.csv";

  // read accel/brake map from csv files
  if (!accel_map_.readAccelMapFromCSV(csv_path_accel_map) || !new_accel_map_.readAccelMapFromCSV(csv_path_accel_map)) {
    ROS_ERROR_STREAM("[accel_brake_map_calibrator]" << "Cannot read map from " << csv_path_accel_map.c_str() << ". stop calculation.");
    return;
  }
  if (!brake_map_.readBrakeMapFromCSV(csv_path_brake_map) || !new_brake_map_.readBrakeMapFromCSV(csv_path_brake_map)) {
    ROS_ERROR_STREAM("[accel_brake_map_calibrator]" << "Cannot read map from " << csv_path_brake_map.c_str() << ". stop calculation.");
    return;
  }
  const std::string output_log_file_ = pnh_.param<std::string>("output_log_file", std::string(""));
  output_log_.open(output_log_file_);
  addIndexToCSV(&output_log_);

  debug_values_.layout.dim.push_back(std_msgs::MultiArrayDimension());
  debug_values_.layout.dim[0].size = num_debug_values_;
  debug_values_.layout.dim[0].label = "length";
  debug_values_.layout.dim[0].stride = num_debug_values_;
  debug_values_.layout.data_offset = 0;
  debug_values_.data.resize(num_debug_values_);

  // input map info
  accel_map_value_ = accel_map_.getAccelMap();
  brake_map_value_ = brake_map_.getBrakeMap();
  accel_vel_index_ = accel_map_.getVelIdx();
  brake_vel_index_ = brake_map_.getVelIdx();
  accel_pedal_index_ = accel_map_.getThrottleIdx();
  brake_pedal_index_ = brake_map_.getBrakeIdx();
  update_accel_map_value_.resize((accel_map_value_.size()));
  update_brake_map_value_.resize((brake_map_value_.size()));
  for (auto &m : update_accel_map_value_) {
    m.resize(accel_map_value_.at(0).size());
  }
  for (auto &m : update_brake_map_value_) {
    m.resize(brake_map_value_.at(0).size());
  }
  map_value_data_.resize(accel_map_value_.size() + brake_map_value_.size() - 1);
  for (auto &m : map_value_data_) {
    m.resize(accel_map_value_.at(0).size());
  }
  std::copy(accel_map_value_.begin(), accel_map_value_.end(), update_accel_map_value_.begin());
  std::copy(brake_map_value_.begin(), brake_map_value_.end(), update_brake_map_value_.begin());

  // init publishers
  update_suggest_pub_ = nh_.advertise<std_msgs::Bool>("output/update_suggest", 1);
  original_map_occ_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/original_occ_map", 1);
  update_map_occ_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/update_occ_map", 1);
  data_ave_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/data_average_occ_map", 1);
  data_std_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/data_std_dev_occ_map", 1);
  data_count_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/data_count_occ_map", 1);
  data_count_with_self_pose_pub_ = nh_.advertise<nav_msgs::OccupancyGrid>(
    "/accel_brake_map_calibrator/debug/data_count_self_pose_occ_map", 1);
  index_pub_ = nh_.advertise<visualization_msgs::MarkerArray>(
    "/accel_brake_map_calibrator/debug/occ_index", 1);
  original_map_raw_pub_ = nh_.advertise<std_msgs::Float32MultiArray>(
    "/accel_brake_map_calibrator/debug/original_raw_map", 1);
  update_map_raw_pub_ = nh_.advertise<std_msgs::Float32MultiArray>(
    "/accel_brake_map_calibrator/output/update_raw_map", 1);
  debug_pub_ = nh_.advertise<std_msgs::Float32MultiArray>(
    "/accel_brake_map_calibrator/output/debug_values", 1);
  current_map_error_pub_ = nh_.advertise<std_msgs::Float32>(
    "/accel_brake_map_calibrator/output/current_map_error", 1);
  updated_map_error_pub_ = nh_.advertise<std_msgs::Float32>(
    "/accel_brake_map_calibrator/output/updated_map_error", 1);
  map_error_ratio_pub_ = nh_.advertise<std_msgs::Float32>(
    "/accel_brake_map_calibrator/output/map_error_ratio", 1);

  // init subscribers
  velocity_sub_ = nh_.subscribe("input/velocity", 1, &AccelBrakeMapCalibrator::callbackVelocity, this);
  steer_sub_ = nh_.subscribe("input/steer", 1, &AccelBrakeMapCalibrator::callbackSteer, this);
  throttle_sub_ = nh_.subscribe("input/throttle", 1, &AccelBrakeMapCalibrator::callbackThrottle, this);
  brake_sub_ = nh_.subscribe("input/brake", 1, &AccelBrakeMapCalibrator::callbackBrake, this);
  imu_sub_ = nh_.subscribe("input/imu", 1, &AccelBrakeMapCalibrator::callbackImu, this);

  save_calibration_server_ = nh_.advertiseService("save_calibration", &AccelBrakeMapCalibrator::saveCalibration, this);

  // init timer
  timer_ = nh_.createTimer(ros::Duration(1.0 / update_hz_), &AccelBrakeMapCalibrator::timerCallback, this);
  // timer_output_csv_ = nh_.createTimer(ros::Duration(30.0), &AccelBrakeMapCalibrator::timerCallbackOutputCSV, this);
}

/**
 * @brief callback function for updating the map values
 * 
 * @param e ros time event, for time capture
 */
void AccelBrakeMapCalibrator::timerCallback(const ros::TimerEvent & e)
{
  update_count_++;
  ROS_INFO_STREAM_THROTTLE(
    2.5,
    "map updating... count: " << update_success_count_ << " / " << update_count_ << "\n\t"
    << "lack_of_data_count: " << lack_of_data_count_ << "\n\t"
    << "failed_to_get_pitch_count: " << failed_to_get_pitch_count_ << "\n\t"
    << "too_large_pitch_count: " << too_large_pitch_count_ << "\n\t"
    << "too_low_speed_count: " << too_low_speed_count_ << "\n\t"
    << "too_large_steer_count: " << too_large_steer_count_ << "\n\t"
    << "too_large_jerk_count: " << too_large_jerk_count_ << "\n\t"
    << "invalid_acc_brake_count: " << invalid_acc_brake_count_ << "\n\t"
    << "too_large_pedal_spd_count: " << too_large_pedal_spd_count_ << "\n\t"
    << "update_fail_count: " << update_fail_count_ << "\n\t");
  
  /* data check */

  // if all data is available
  if (!twist_ptr_ || !steer_ptr_ || !accel_pedal_ptr_ || !brake_pedal_ptr_ ||
      !delayed_accel_pedal_ptr_ || !delayed_brake_pedal_ptr_ || 
      (get_pitch_method_==GET_PITCH_METHOD::IMU && !imu_pitch_ptr_))
  {
    lack_of_data_count_++;
    ROS_WARN_STREAM_THROTTLE(5.0, "lack of topics (twist, steer, pedal, imu)");
    return;
  }

  // if all data is time satisfied
  if (isTimeout(twist_ptr_->header.stamp, e.current_real, timeout_sec_) ||
      isTimeout(steer_ptr_->data_time, e.current_real, timeout_sec_) ||
      isTimeout(accel_pedal_ptr_->data_time, e.current_real, timeout_sec_) ||
      isTimeout(brake_pedal_ptr_->data_time, e.current_real, timeout_sec_) 
      )
  {
    ROS_WARN_STREAM_THROTTLE(5.0, "timeout for topics (twist, steer, pedal, imu)");
    lack_of_data_count_++;
    return;
  }

  // if pitch is available
  if (!getCurrentPitch(&pitch_)) {
    ROS_WARN_STREAM_THROTTLE(5.0, "cannot get pitch");
    failed_to_get_pitch_count_++;
    return;
  }

  // write data to log
  if (pedal_accel_graph_output_) {
    addLogToCSV(
      &output_log_, twist_ptr_->header.stamp.toSec(), twist_ptr_->twist.linear.x,
      acceleration_, getPitchCompensatedAcceleration(), delayed_accel_pedal_ptr_->data,
      delayed_brake_pedal_ptr_->data, accel_pedal_speed_, brake_pedal_speed_, pitch_,
      steer_ptr_->data, jerk_, full_original_accel_rmse_, part_original_accel_rmse_,
      new_accel_rmse_);
  }

  // publish map 
  publishMap(accel_map_value_, brake_map_value_, "original");
  publishMap(update_accel_map_value_, update_brake_map_value_, "update");
  publishCountMap();
  publishIndex();
  publishUpdateSuggestFlag();
  debug_pub_.publish(debug_values_);
  publishFloat32("current_map_error", part_original_accel_rmse_);
  publishFloat32("updated_map_error", new_accel_rmse_);
  publishFloat32(
    "map_error_ratio",
    part_original_accel_rmse_ != 0.0 ? new_accel_rmse_ / part_original_accel_rmse_ : 1.0);

  // update map values

  debug_values_.data.at(SUCCESS_TO_UPDATE) = false;
  update_success_ = false;

  // twist check
  if (twist_ptr_->twist.linear.x < velocity_min_threshold_) {
    too_low_speed_count_++;
    return;
  }

  // accel / brake map evaluation
  executeEvaluation();

  // pitch check
  if (std::abs(pitch_) > max_pitch_threshold_) {
    too_large_pitch_count_++;
    return;
  }

  // steer check
  if (std::abs(steer_ptr_->data) > max_steer_threshold_) {
    too_large_steer_count_++;
    return;
  }

  // jerk check
  if (std::abs(jerk_) > max_jerk_threshold_) {
    too_large_jerk_count_++;
    return;
  }

  // pedal check
  if (
    delayed_accel_pedal_ptr_->data > std::numeric_limits<double>::epsilon() &&
    delayed_brake_pedal_ptr_->data > std::numeric_limits<double>::epsilon()) {
      ROS_DEBUG_STREAM_THROTTLE(5.0, "invalid pedal value (Both accel and brake are not zero.");
    invalid_acc_brake_count_++;
    return;
  }

  // pedal speed check
  if (std::abs(accel_pedal_speed_) > pedal_velocity_thresh_ ||
      std::abs(brake_pedal_speed_) > pedal_velocity_thresh_) {
    too_large_pedal_spd_count_++;
    return;
  }

  // update map values
  if (updateAccelBrakeMap()) {
    update_success_count_++;
    debug_values_.data.at(SUCCESS_TO_UPDATE) = true;
    update_success_ = true;
  } else {
    update_fail_count_++;
  }
}

/**
 * @brief callback function to update csv file
 * 
 * @param e ros timer event
 */
void AccelBrakeMapCalibrator::timerCallbackOutputCSV(const ros::TimerEvent &e)
{
  if (!auto_update_) return;
  // write accel/ brake map to file
  const auto ros_time = std::to_string(e.current_real.sec);
  writeMapToCSV(accel_vel_index_, accel_pedal_index_, update_accel_map_value_, output_accel_file_);
  writeMapToCSV(brake_vel_index_, brake_pedal_index_, update_brake_map_value_, output_brake_file_);
  if (progress_file_output_) {
    writeMapToCSV(
      accel_vel_index_, accel_pedal_index_, update_accel_map_value_,
      output_accel_file_ + "_" + ros_time);
    writeMapToCSV(
      brake_vel_index_, brake_pedal_index_, update_brake_map_value_,
      output_brake_file_ + "_" + ros_time);
    writeMapToCSV(
      accel_vel_index_, accel_pedal_index_, accel_map_value_, output_accel_file_ + "_original");
    writeMapToCSV(
      brake_vel_index_, brake_pedal_index_, brake_map_value_, output_brake_file_ + "_original");
  }

  // update newest accel / brake map
  // check file existence
  std::ifstream af(output_accel_file_);
  std::ifstream bf(output_brake_file_);
  if (!af.is_open() || !bf.is_open()) {
    ROS_WARN("Accel map or brake map does not exist");
    return;
  }

  new_accel_map_ = AccelMap();
  if (!new_accel_map_.readAccelMapFromCSV(output_accel_file_)) {
    ROS_WARN("Cannot read accelmap. csv path = %s. ", output_accel_file_.c_str());
  }
  new_brake_map_ = BrakeMap();
  if (!new_brake_map_.readBrakeMapFromCSV(output_brake_file_)) {
    ROS_WARN("Cannot read brakemap. csv path = %s. ", output_brake_file_.c_str());
  }
}

bool AccelBrakeMapCalibrator::saveCalibration(accel_brake_map_calibrator::SaveCalibration::Request &req,
                        accel_brake_map_calibrator::SaveCalibration::Response &res)
{
  const auto ros_time = std::to_string(ros::Time::now().toSec());
  writeMapToCSV(accel_vel_index_, accel_pedal_index_, update_accel_map_value_, req.path_to_save.data+ "/accel_map.csv");
  writeMapToCSV(brake_vel_index_, brake_pedal_index_, update_brake_map_value_, req.path_to_save.data+ "/brake_map.csv");
  if (progress_file_output_) {
    writeMapToCSV(
      accel_vel_index_, accel_pedal_index_, update_accel_map_value_,
      output_accel_file_ + "_" + ros_time);
    writeMapToCSV(
      brake_vel_index_, brake_pedal_index_, update_brake_map_value_,
      output_brake_file_ + "_" + ros_time);
    writeMapToCSV(
      accel_vel_index_, accel_pedal_index_, accel_map_value_, output_accel_file_ + "_original");
    writeMapToCSV(
      brake_vel_index_, brake_pedal_index_, brake_map_value_, output_brake_file_ + "_original");
  }

  // update newest accel / brake map
  // check file existence
  std::ifstream af(output_accel_file_);
  std::ifstream bf(output_brake_file_);
  if (!af.is_open() || !bf.is_open()) {
    ROS_WARN("Accel map or brake map does not exist");
    return false;
  }

  new_accel_map_ = AccelMap();
  if (!new_accel_map_.readAccelMapFromCSV(output_accel_file_)) {
    ROS_WARN("Cannot read accelmap. csv path = %s. ", output_accel_file_.c_str());
  }
  new_brake_map_ = BrakeMap();
  if (!new_brake_map_.readBrakeMapFromCSV(output_brake_file_)) {
    ROS_WARN("Cannot read brakemap. csv path = %s. ", output_brake_file_.c_str());
  }
  res.is_saved.data = "calibration Map saved to " + req.path_to_save.data+ "/accel_map.csv" + " and " + req.path_to_save.data+ "/brake_map.csv" +".";

  return true;

}

/**
 * @brief write value map into csv files
 * 
 * @param vel_index velocity index
 * @param pedal_index pedal index
 * @param value_map value map
 * @param filename csv filename
 * @return true if success, 
 * @return false if cannot open file
 */
bool AccelBrakeMapCalibrator::writeMapToCSV(std::vector<double> vel_index, std::vector<double> pedal_index, 
  std::vector<std::vector<double>> value_map, std::string filename)
{
  if (update_success_count_ == 0) {
    ROS_ERROR("update_success_counts is 0.");
    return false;
  }

  std::ofstream csv_file(filename);

  if (!csv_file.is_open()) {
    ROS_WARN("Failed to open csv file : %s", filename.c_str());
    return false;
  }

  csv_file << "default,";
  for (std::size_t v = 0; v < vel_index.size(); v++) {
    csv_file << vel_index.at(v);
    if (v != vel_index.size() - 1) {
      csv_file << ",";
    }
  }
  csv_file << "\n";

  for (std::size_t p = 0; p < pedal_index.size(); p++) {
    csv_file << pedal_index.at(p) << ",";
    for (std::size_t v = 0; v < vel_index.size(); v++) {
      csv_file << std::setprecision(3) << value_map.at(p).at(v);
      if (v != vel_index.size() - 1) {
        csv_file << ",";
      }
    }
    csv_file << "\n";
  }
  csv_file.close();
  ROS_DEBUG_STREAM("output map to " << filename);
  return true;
}

/**
 * @brief update the update accel /brake map value
 * 
 * @return true if success
 * @return false if not match the velocity or pedal value
 */
bool AccelBrakeMapCalibrator::updateAccelBrakeMap()
{
  // get pedal index
  bool accel_mode = false;
  int accel_pedal_index = 0;
  int brake_pedal_index = 0;
  int accel_vel_index = 0;
  int brake_vel_index = 0;

  if (delayed_accel_pedal_ptr_->data > std::numeric_limits<double>::epsilon()) {
    accel_mode = true;
  } else {
    accel_mode = false;
  }

  if (
    accel_mode && !indexValueSearch(
                    accel_pedal_index_, delayed_accel_pedal_ptr_->data, pedal_diff_threshold_,
                    &accel_pedal_index)) {
    // not match accel pedal output to pedal value in index
    return false;
  }

  if (
    !accel_mode && !indexValueSearch(
                     brake_pedal_index_, delayed_brake_pedal_ptr_->data, pedal_diff_threshold_,
                     &brake_pedal_index)) {
    // not match accel pedal output to pedal value in index
    return false;
  }

  if (
    accel_mode &&
    !indexValueSearch(
      accel_vel_index_, twist_ptr_->twist.linear.x, velocity_diff_threshold_, &accel_vel_index)) {
    // not match current velocity to velocity value in index
    return false;
  }

  if (
    !accel_mode &&
    !indexValueSearch(
      brake_vel_index_, twist_ptr_->twist.linear.x, velocity_diff_threshold_, &brake_vel_index)) {
    // not match current velocity to velocity value in index
    return false;
  }

  // update map
  executeUpdate(accel_mode, accel_pedal_index, accel_vel_index, brake_pedal_index, brake_vel_index);

  // when update 0 pedal index, update another map
  if (accel_mode && accel_pedal_index == 0) {
    // copy accel map value to brake map value
    update_brake_map_value_.at(accel_pedal_index).at(accel_vel_index) =
      update_accel_map_value_.at(accel_pedal_index).at(accel_vel_index);
  } else if (!accel_mode && brake_pedal_index == 0) {
    // copy brake map value to accel map value
    update_accel_map_value_.at(brake_pedal_index).at(brake_vel_index) =
      update_brake_map_value_.at(brake_pedal_index).at(brake_vel_index);
  }

  // take consistency of map
  takeConsistencyOfAccelMap();
  takeConsistencyOfBrakeMap();

  return true;
}

/**
 * @brief preserve the consistency of the update accel map value
 * 
 */
void AccelBrakeMapCalibrator::takeConsistencyOfAccelMap()
{
  const double bit = 1e-03;
  for (std::size_t ped_idx = 0; ped_idx < update_accel_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; vel_idx < update_accel_map_value_.at(0).size() - 1; vel_idx++) {
      const double current_acc = update_accel_map_value_.at(ped_idx).at(vel_idx);
      const double next_ped_acc = update_accel_map_value_.at(ped_idx + 1).at(vel_idx);
      const double next_vel_acc = update_accel_map_value_.at(ped_idx).at(vel_idx + 1);

      if (current_acc <= next_vel_acc) {
        // the higher the velocity, the lower the acceleration
        update_accel_map_value_.at(ped_idx).at(vel_idx + 1) = current_acc - bit;
      }

      if (current_acc >= next_ped_acc) {
        // the higher the accel pedal, the higher the acceleration
        update_accel_map_value_.at(ped_idx + 1).at(vel_idx) = current_acc + bit;
      }
    }
  }
}

/**
 * @brief preserve the consistency of the update brake map value
 * 
 */
void AccelBrakeMapCalibrator::takeConsistencyOfBrakeMap()
{
  const double bit = 1e-03;
  for (std::size_t ped_idx = 0; ped_idx < update_brake_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; vel_idx < update_brake_map_value_.at(0).size() - 1; vel_idx++) {
      const double current_acc = update_brake_map_value_.at(ped_idx).at(vel_idx);
      const double next_ped_acc = update_brake_map_value_.at(ped_idx + 1).at(vel_idx);
      const double next_vel_acc = update_brake_map_value_.at(ped_idx).at(vel_idx + 1);

      if (current_acc <= next_vel_acc) {
        // the higher the velocity, the lower the acceleration
        update_brake_map_value_.at(ped_idx).at(vel_idx + 1) = current_acc - bit;
      }

      if (current_acc <= next_ped_acc) {
        // the higher the brake pedal, the lower the acceleration
        update_brake_map_value_.at(ped_idx + 1).at(vel_idx) = current_acc - bit;
      }
    }
  }
}

/**
 * @brief update map
 * 
 */
void AccelBrakeMapCalibrator::executeUpdate(bool accel_mode, int accel_pedal_index, int accel_vel_index, 
  int brake_pedal_index, int brake_vel_index)
{
const double measured_acc = acceleration_ - getPitchCompensatedAcceleration();
  const double map_acc = accel_mode
                           ? update_accel_map_value_.at(accel_pedal_index).at(accel_vel_index)
                           : update_brake_map_value_.at(brake_pedal_index).at(brake_vel_index);
  ROS_DEBUG_STREAM("measured_acc: " << measured_acc << ", map_acc: " << map_acc);

  if (update_method_ == UPDATE_METHOD::UPDATE_OFFSET_EACH_CELL) {
    updateEachValOffset(
      accel_mode, accel_pedal_index, accel_vel_index, brake_pedal_index, brake_vel_index,
      measured_acc, map_acc);
  } else if (update_method_ == UPDATE_METHOD::UPDATE_OFFSET_TOTAL) {
    updateTotalMapOffset(measured_acc, map_acc);
  }

  // add accel data to map
  accel_mode ? map_value_data_.at(getUnifiedIndexFromAccelBrakeIndex(true, accel_pedal_index))
                 .at(accel_vel_index)
                 .emplace_back(measured_acc)
             : map_value_data_.at(getUnifiedIndexFromAccelBrakeIndex(false, brake_pedal_index))
                 .at(brake_vel_index)
                 .emplace_back(measured_acc);
}

/**
 * @brief update the update accel / brake map values based on a total offset
 * 
 */
void AccelBrakeMapCalibrator::updateTotalMapOffset(double measured_acc, double map_acc)
{
  /* calculate adaptive map offset */
  const double phi = 1.0;
  covariance_ = (covariance_ - (covariance_ * phi * phi * covariance_) /
                                 (forgetting_factor_ + phi * covariance_ * phi)) /
                forgetting_factor_;

  const double coef = (covariance_ * phi) / (forgetting_factor_ + phi * covariance_ * phi);
  const double error_map_offset = measured_acc - map_acc;
  map_offset_ = map_offset_ + coef * error_map_offset;

  ROS_DEBUG_STREAM(
    "map_offset_ = " << map_offset_ << "\t"
                     << "covariance = " << covariance_);

  /* update map */
  for (std::size_t ped_idx = 0; ped_idx < update_accel_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; vel_idx < update_accel_map_value_.at(0).size() - 1; vel_idx++) {
      update_accel_map_value_.at(ped_idx).at(vel_idx) =
        accel_map_value_.at(ped_idx).at(vel_idx) + map_offset_;
    }
  }
  for (std::size_t ped_idx = 0; ped_idx < update_brake_map_value_.size() - 1; ped_idx++) {
    for (std::size_t vel_idx = 0; vel_idx < update_brake_map_value_.at(0).size() - 1; vel_idx++) {
      update_brake_map_value_.at(ped_idx).at(vel_idx) =
        brake_map_value_.at(ped_idx).at(vel_idx) + map_offset_;
    }
  }
}

/**
 * @brief update the offsets and update accel / brake map values based on the values in each index and velocity
 * 
 */
bool AccelBrakeMapCalibrator::updateEachValOffset(bool accel_mode, int accel_pedal_index, 
  int accel_vel_index, int brake_pedal_index, int brake_vel_index, double measured_acc, double map_acc)
{
  // pre-defined
  static std::vector<std::vector<double>> map_offset_vec_(
    accel_map_value_.size() + brake_map_value_.size() - 1,
    std::vector<double>(accel_map_value_.at(0).size(), map_offset_));
  static std::vector<std::vector<double>> covariance_vec_(
    accel_map_value_.size() + brake_map_value_.size() - 1,
    std::vector<double>(accel_map_value_.at(0).size(), covariance_));

  const int vel_idx = accel_mode ? accel_vel_index : brake_vel_index;
  int ped_idx = accel_mode ? accel_pedal_index : brake_pedal_index;
  ped_idx = getUnifiedIndexFromAccelBrakeIndex(accel_mode, ped_idx);
  double map_offset = map_offset_vec_.at(ped_idx).at(vel_idx);
  double covariance = covariance_vec_.at(ped_idx).at(vel_idx);

  /* calculate adaptive map offset */
  const double phi = 1.0;
  covariance = (covariance - (covariance * phi * phi * covariance) /
                               (forgetting_factor_ + phi * covariance * phi)) /
               forgetting_factor_;

  const double coef = (covariance * phi) / (forgetting_factor_ + phi * covariance * phi);

  const double error_map_offset = measured_acc - map_acc;
  map_offset = map_offset + coef * error_map_offset;

  ROS_DEBUG_STREAM(
    "index: " << ped_idx << ", " << vel_idx << ": map_offset_ = "
              << map_offset_vec_.at(ped_idx).at(vel_idx) << " -> " << map_offset << "\t"
              << " covariance = " << covariance);

  /* input calculated result and update map */
  map_offset_vec_.at(ped_idx).at(vel_idx) = map_offset;
  covariance_vec_.at(ped_idx).at(vel_idx) = covariance;
  if (accel_mode) {
    update_accel_map_value_.at(accel_pedal_index).at(accel_vel_index) =
      accel_map_value_.at(accel_pedal_index).at(accel_vel_index) + map_offset;
  } else {
    update_brake_map_value_.at(brake_pedal_index).at(brake_vel_index) =
      brake_map_value_.at(brake_pedal_index).at(brake_vel_index) + map_offset;
  }
  return true;
}

/**
 * @brief Search the index of the first value whose difference to the reference is smaller than the threshold 
 * 
 * @param value_index vetor of the values to be searched
 * @param value reference value
 * @param value_thresh value threshold
 * @param searched_index index of the value in the vector
 * @return true successfully find the index
 * @return false no value is satisfied
 */
bool AccelBrakeMapCalibrator::indexValueSearch(std::vector<double> value_index, double value, 
  double value_thresh, int *searched_index)
{
  for (std::size_t i = 0; i < value_index.size(); i++) {
    const double diff_value = std::abs(value_index.at(i) - value);
    if (diff_value <= value_thresh) {
      *searched_index = i;
      return true;
    }
  }
  return false;
}

/**
 * @brief calculate squared error of the acceleration and the mse in a period
 * 
 */
void AccelBrakeMapCalibrator::executeEvaluation()
{
  const double full_orig_accel_sq_error = calculateAccelSquaredError(
    delayed_accel_pedal_ptr_->data, delayed_brake_pedal_ptr_->data, twist_ptr_->twist.linear.x,
    accel_map_, brake_map_);
  pushDataToVec(full_orig_accel_sq_error, full_mse_que_size_, &full_original_accel_mse_que_);
  full_original_accel_rmse_ = getAverage(full_original_accel_mse_que_);

  const double part_orig_accel_sq_error = calculateAccelSquaredError(
    delayed_accel_pedal_ptr_->data, delayed_brake_pedal_ptr_->data, twist_ptr_->twist.linear.x,
    accel_map_, brake_map_);
  pushDataToVec(part_orig_accel_sq_error, part_mse_que_size_, &part_original_accel_mse_que_);
  part_original_accel_rmse_ = getAverage(part_original_accel_mse_que_);

  const double new_accel_sq_error = calculateAccelSquaredError(
    delayed_accel_pedal_ptr_->data, delayed_brake_pedal_ptr_->data, twist_ptr_->twist.linear.x,
    new_accel_map_, new_brake_map_);
  pushDataToVec(new_accel_sq_error, part_mse_que_size_, &new_accel_mse_que_);
  new_accel_rmse_ = getAverage(new_accel_mse_que_);
}

/**
 * @brief calculate the square error between the map acceleration and the mesured acceleration
 * 
 */
double AccelBrakeMapCalibrator::calculateAccelSquaredError(double throttle, double brake, double vel, 
  AccelMap &accel_map, BrakeMap &brake_map)
{
  const double estimated_acc = calculateEstimatedAcc(throttle, brake, vel, accel_map, brake_map);
  const double measured_acc = acceleration_ - getPitchCompensatedAcceleration();
  const double dif_acc = measured_acc - estimated_acc;
  return dif_acc * dif_acc;
}

/**
 * @brief get estimated acceleration from the map
 * 
 */
double AccelBrakeMapCalibrator::calculateEstimatedAcc(double throttle, double brake, double vel, 
  AccelMap &accel_map, BrakeMap &brake_map)
{
  const double pedal = throttle - brake;

  double estimated_acc = 0.0;
  if (pedal > 0.0) {
    accel_map.getAcceleration(pedal, vel, estimated_acc);
  } else {
    brake_map.getAcceleration(-pedal, vel, estimated_acc);
  }

  return estimated_acc;
}

/**
 * @brief publish float val based on the publish type
 * 
 * @param publish_type choose from "map_error_ratio", "current_map_error" and "updated_map_error"
 * @param val value to be published
 */
void AccelBrakeMapCalibrator::publishFloat32(std::string publish_type, double val)
{
  std_msgs::Float32 msg;
  msg.data = val;
  if (publish_type == "current_map_error") {
    current_map_error_pub_.publish(msg);
  } else if (publish_type == "updated_map_error") {
    updated_map_error_pub_.publish(msg);
  } else if (publish_type == "map_error_ratio"){
    map_error_ratio_pub_.publish(msg);
  } else {
    ROS_ERROR("[accel_brake_map_calibrator] Invalid publish type in function publishFloat32.");
  }
}

/**
 * @brief publish update suggest, true if suggested
 * 
 */
void AccelBrakeMapCalibrator::publishUpdateSuggestFlag()
{
  std_msgs::Bool update_suggest;

  if (new_accel_mse_que_.size() < part_mse_que_size_ / 2) {
    // lack of data
    update_suggest.data = false;
  } else {
    const double rmse_rate = new_accel_rmse_ / part_original_accel_rmse_;
    update_suggest.data = (rmse_rate < update_suggest_thresh_);
    if (update_suggest.data) {
      ROS_WARN_STREAM_THROTTLE(
        5.0,
        "suggest to update accel/brake map. evaluation score = " << rmse_rate);
    }
  }
  update_suggest_pub_.publish(update_suggest);
}

/**
 * @brief publish index value to rviz
 * 
 */
void AccelBrakeMapCalibrator::publishIndex()
{
  visualization_msgs::MarkerArray markers;
  const double h = accel_map_value_.size() + brake_map_value_.size() -
                   1;  // pedal (accel_map_value(0) and brake_map_value(0) is same.)
  const double w = accel_map_value_.at(0).size();  // velocity

  visualization_msgs::Marker marker;
  marker.header.frame_id = "base_link";
  marker.header.stamp = ros::Time::now();
  marker.type = marker.TEXT_VIEW_FACING;
  marker.action = marker.ADD;
  std_msgs::ColorRGBA color;
  color.a = 0.999;
  color.b = 0.1;
  color.g = 0.1;
  color.r = 0.1;
  marker.color = color;
  marker.frame_locked = true;
  marker.pose.position.z = 0.0;
  marker.pose.orientation.x = 0.0;
  marker.pose.orientation.y = 0.0;
  marker.pose.orientation.z = 0.0;
  marker.pose.orientation.w = 1.0;

  // pedal value
  for (int pedal_idx = 0; pedal_idx < h; pedal_idx++) {
    const double pedal_value = getPedalValueFromUnifiedIndex(pedal_idx);
    marker.ns = "occ_pedal_index";
    marker.id = pedal_idx;
    marker.pose.position.x = -map_resolution_ * 0.5;
    marker.pose.position.y = map_resolution_ * (0.5 + pedal_idx);
    marker.scale.z = 0.05;
    std::stringstream stream;
    stream << std::fixed << std::setprecision(2) << pedal_value;
    marker.text = stream.str();
    markers.markers.push_back(marker);
  }

  // pedal index name
  marker.ns = "occ_pedal_index";
  marker.id = h;
  marker.pose.position.x = -map_resolution_ * 0.5;
  marker.pose.position.y = map_resolution_ * (0.5 + h);
  marker.scale.z = 0.05;
  marker.text = "(accel_pedal/brake_pedal)";
  markers.markers.push_back(marker);

  // velocity value
  for (int vel_idx = 0; vel_idx < w; vel_idx++) {
    const double vel_value = accel_vel_index_.at(vel_idx);
    marker.ns = "occ_vel_index";
    marker.id = vel_idx;
    marker.pose.position.x = map_resolution_ * (0.5 + vel_idx);
    marker.pose.position.y = -map_resolution_ * 0.2;
    marker.scale.z = 0.1;
    std::stringstream stream;
    stream << std::fixed << std::setprecision(2) << vel_value;
    marker.text = stream.str() + "m/s";
    markers.markers.push_back(marker);
  }

  // velocity index name
  marker.ns = "occ_vel_index";
  marker.id = w;
  marker.pose.position.x = map_resolution_ * (0.5 + w);
  marker.pose.position.y = -map_resolution_ * 0.2;
  marker.scale.z = 0.1;
  marker.text = "(velocity)";
  markers.markers.push_back(marker);

  index_pub_.publish(markers);
}

/**
 * @brief Get pedal value from unified index
 * 
 * @param index unified index
 * @return double pedal value
 */
double AccelBrakeMapCalibrator::getPedalValueFromUnifiedIndex(std::size_t index)
{
  if (index < brake_pedal_index_.size()) {
    // brake index ( minus )
    return -brake_pedal_index_.at(brake_pedal_index_.size() - index - 1);
  } else {
    // input accel map value
    return accel_pedal_index_.at(index - brake_pedal_index_.size() + 1);
  }
}

/**
 * @brief publish average map / standard dev map / count map, and update states
 * 
 */
void AccelBrakeMapCalibrator::publishCountMap()
{
  if (accel_map_value_.at(0).size() != brake_map_value_.at(0).size()) {
    ROS_ERROR_STREAM(
      "Invalid map. The number of velocity index of accel map and brake map is different.");
    return;
  }
  const double h = accel_map_value_.size() + brake_map_value_.size() -
                   1;  // pedal (accel_map_value(0) and brake_map_value(0) is same.)
  const double w = accel_map_value_.at(0).size();  // velocity
  const int8_t MAX_OCC_VALUE = 100;

  std::vector<int8_t> count_map, ave_map, std_map;
  count_map.resize(h * w);
  ave_map.resize(h * w);
  std_map.resize(h * w);

  for (int i = 0; i < h; i++) {
    for (int j = 0; j < w; j++) {
      const auto data_vec = map_value_data_.at(i).at(j);
      if (data_vec.empty()) {
        // input *UNKNOWN* value
        count_map.at(i * w + j) = -1;
        ave_map.at(i * w + j) = -1;
      } else {
        const auto count_rate =
          MAX_OCC_VALUE * (static_cast<double>(data_vec.size()) / max_data_count_);
        count_map.at(i * w + j) = static_cast<int8_t>(
          std::max(std::min(static_cast<int>(MAX_OCC_VALUE), static_cast<int>(count_rate)), 0));
        // calculate average
        {
          const double average = getAverage(data_vec);
          int8_t int_average = static_cast<uint8_t>(
            MAX_OCC_VALUE * ((average - min_accel_) / (max_accel_ - min_accel_)));
          ave_map.at(i * w + j) = std::max(std::min(MAX_OCC_VALUE, int_average), (int8_t)0);
        }
        // calculate standard deviation
        {
          const double std_dev = getStandardDeviation(data_vec);
          const double max_std_dev = 0.2;
          const double min_std_dev = 0.0;
          int8_t int_std_dev = static_cast<uint8_t>(
            MAX_OCC_VALUE * ((std_dev - min_std_dev) / (max_std_dev - min_std_dev)));
          std_map.at(i * w + j) = std::max(std::min(MAX_OCC_VALUE, int_std_dev), (int8_t)0);
        }
      }
    }
  }

  // publish average map / standard dev map / count map
  data_ave_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, ave_map));
  data_std_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, std_map));
  data_count_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, count_map));

  // publish count with self pos map
  int nearest_pedal_idx = nearestPedalSearch();
  int nearest_vel_idx = nearestVelSearch();

  // input self pose (to max value / min value ) and publish this
  update_success_
    ? count_map.at(nearest_pedal_idx * w + nearest_vel_idx) = std::numeric_limits<int8_t>::max()
    : count_map.at(nearest_pedal_idx * w + nearest_vel_idx) = std::numeric_limits<int8_t>::min();
  data_count_with_self_pose_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, count_map));
}

/**
 * @brief Get the nearest velocity index based on the twist pointer
 * 
 * @return int earest velocity index
 */
int AccelBrakeMapCalibrator::nearestVelSearch()
{
  const double current_vel = twist_ptr_->twist.linear.x;
  return nearestValueSearch(accel_vel_index_, current_vel);
}

/**
 * @brief Get the nearest pedal index based on the delayed pedal pointer
 * 
 * @return int nearset pedal index
 */
int AccelBrakeMapCalibrator::nearestPedalSearch()
{
  bool accel_mode;

  if (delayed_accel_pedal_ptr_->data > std::numeric_limits<double>::epsilon()) {
    accel_mode = true;
  } else {
    accel_mode = false;
  }

  int nearest_idx;
  if (accel_mode) {
    nearest_idx = nearestValueSearch(accel_pedal_index_, delayed_accel_pedal_ptr_->data);
  } else {
    nearest_idx = nearestValueSearch(brake_pedal_index_, delayed_brake_pedal_ptr_->data);
  }

  return getUnifiedIndexFromAccelBrakeIndex(accel_mode, nearest_idx);
}

/**
 * @brief return the unified index of the accel brake map based on the accel / brake map index
 * 
 * @param accel_map true if the index is from accel map
 * @param index index of the accel / brake map
 * @return int unified index of the accel brake map
 */
int AccelBrakeMapCalibrator::getUnifiedIndexFromAccelBrakeIndex(bool accel_map, std::size_t index)
{
  // accel_map=true: accel_map; accel_map=false: brake_map
  if (accel_map) {
    // accel map
    return index + brake_map_value_.size() - 1;
  } else {
    // brake map
    return brake_map_value_.size() - index - 1;
  }
}

/**
 * @brief Given a vector of values and a value reference, return the index of the value closest to the reference
 * 
 * @param value_index vector of the values
 * @param value reference value
 * @return int index of the closest value
 */
int AccelBrakeMapCalibrator::nearestValueSearch(std::vector<double> value_index, double value)
{
  double max_dist = std::numeric_limits<double>::max();
  int nearest_idx = 0;

  for (std::size_t i = 0; i < value_index.size(); i++) {
    const double dist = std::fabs(value - value_index.at(i));
    if (max_dist > dist) {
      nearest_idx = i;
      max_dist = dist;
    }
  }
  return nearest_idx;
}

/**
 * @brief get standard deviation value of the vector, 0 if empty
 * 
 */
double AccelBrakeMapCalibrator::getStandardDeviation(const std::vector<double> &vec)
{
  if (vec.empty()) return 0.0;
  const double ave = getAverage(vec);
  double summary = 0.0;
  for (auto i : vec) {
    summary += std::pow(i - ave, 2);
  }
  return std::sqrt(summary / vec.size());
}

/**
 * @brief get the average value of a vector, 0 if empty
 * 
 */
double AccelBrakeMapCalibrator::getAverage(const std::vector<double> &vec)
{
  if (vec.empty()) return 0.0;
  double summary = 0.0;
  for (auto i : vec) {
    summary += i;
  }
  return summary / vec.size();
}

/**
 * @brief publish OccupancyGrid map and raw data of the "original" or "update" map value
 * 
 * @param accel_map_value vector of the acceleration map value
 * @param brake_map_value vector of the brake map value
 * @param publish_type different publisher, chosen from "orginal" and "update"
 */
void AccelBrakeMapCalibrator::publishMap(const std::vector<std::vector<double>> accel_map_value,
  const std::vector<std::vector<double>> brake_map_value, const std::string publish_type)
{
  if (accel_map_value.at(0).size() != brake_map_value.at(0).size()) {
    ROS_ERROR("[accel_brake_map_calibrator]Invalid map. The number of velocity index of accel map and brake map is different.");
    return;
  }
  const double h = accel_map_value.size() + brake_map_value.size() - 1;
  const double w = accel_map_value.at(0).size();

  // publish occupancy grid map
  const int8_t MAX_OCC_VALUE = 100;
  std::vector<int8_t> int_map_value;
  int_map_value.resize(h * w);
  std::vector<float> vec(h * w, 0);
  for (int i =0; i < h; i++) {
    const auto i_column = getMapColumnFromUnifiedIndex(accel_map_value, brake_map_value, i);
    for (int j = 0; j < w; j++) {
      const double value = i_column.at(j);
      vec.at(i * w + j) = static_cast<float>(value);
      int8_t int_value = static_cast<uint8_t>(MAX_OCC_VALUE * ((value - min_accel_) / (max_accel_ - min_accel_)));
      int_map_value.at(i * w + j) = std::max(std::min(MAX_OCC_VALUE, int_value), (int8_t)0);
    }
  }

  if (publish_type == "original") {
    original_map_occ_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, int_map_value));
  } else {
    update_map_occ_pub_.publish(getOccMsg("base_link", h, w, map_resolution_, int_map_value));
  }

  // publish raw map
  std_msgs::Float32MultiArray float_map;

  float_map.layout.dim.push_back(std_msgs::MultiArrayDimension());
  float_map.layout.dim.push_back(std_msgs::MultiArrayDimension());
  float_map.layout.dim[0].label = "height";
  float_map.layout.dim[1].label = "width";
  float_map.layout.dim[0].size = h;
  float_map.layout.dim[1].size = w;
  float_map.layout.dim[0].stride = h * w;
  float_map.layout.dim[1].stride = w;
  float_map.layout.data_offset = 0;
  float_map.data = vec;
  if (publish_type == "original") {
    original_map_raw_pub_.publish(float_map);
  } else {
    update_map_raw_pub_.publish(float_map);
  }
}

/**
 * @brief return the OccupancyGrid ros message based on the data given
 * 
 */
nav_msgs::OccupancyGrid AccelBrakeMapCalibrator::getOccMsg(
  std::string frame_id, double height, double width, double resolution, const std::vector<int8_t> &map_value)
{
  nav_msgs::OccupancyGrid occ;
  occ.header.frame_id = frame_id;
  occ.header.stamp = ros::Time::now();
  occ.info.height = height;
  occ.info.width = width;
  occ.info.map_load_time = ros::Time::now();
  occ.info.origin.position.x = 0;
  occ.info.origin.position.y = 0;
  occ.info.origin.position.z = 0;
  occ.info.origin.orientation.x = 0;
  occ.info.origin.orientation.y = 0;
  occ.info.origin.orientation.z = 0;
  occ.info.origin.orientation.w = 1;
  occ.info.resolution = resolution;
  occ.data = map_value;
  return occ;
}

/**
 * @brief get the column of the map value from the unified index
 * 
 * @param accel_map_value acceleration map index
 * @param brake_map_value brake map index
 * @param index unified index value (started from accel_map)
 * @return std::vector<double> the column of the map value
 */
std::vector<double> AccelBrakeMapCalibrator::getMapColumnFromUnifiedIndex(
  const std::vector<std::vector<double>> &accel_map_value, const std::vector<std::vector<double>> &brake_map_value, std::size_t index)
{
  if (index < brake_map_value.size()) {
    // input brake map value
    return brake_map_value.at(brake_map_value.size() - index - 1);
  } else {
    // input accel map value
    return accel_map_value.at(index - brake_map_value.size() + 1);
  }
}

/**
 * @brief get the negative acceleration caused by the gravity
 * 
 * @return double negative acceleration caused by the gravity
 */
double AccelBrakeMapCalibrator::getPitchCompensatedAcceleration()
{
  constexpr double gravity = 9.80665;
  return gravity * std::sin(pitch_);
}

/**
 * @brief push data value to csv file
 * 
 */
void AccelBrakeMapCalibrator::addLogToCSV(
  std::ofstream * csv_file, const double & timestamp, const double velocity, const double accel,
  const double pitched_accel, const double accel_pedal, const double brake_pedal,
  const double accel_pedal_speed, const double brake_pedal_speed, const double pitch,
  const double steer, const double jerk, const double full_original_accel_mse,
  const double part_original_accel_mse, const double new_accel_mse)
{
  const double rmse_rate = 
    part_original_accel_mse == 0.0 ? 1.0 : (new_accel_mse / part_original_accel_mse);
  *csv_file << timestamp << ", " << velocity << ", " << accel << ", " << pitched_accel << ", "
            << accel - pitched_accel << ", " << accel_pedal << ", " << brake_pedal << ", "
            << accel_pedal_speed << ", " << brake_pedal_speed << ", " << pitch << ", " << steer
            << ", " << jerk << ", " << full_original_accel_mse << ", " << part_original_accel_mse
            << ", " << new_accel_mse << "," << rmse_rate << std::endl;
}

/**
 * @brief get current pitch based on the specified method
 * 
 * @param pitch address of the pitch value
 * @return true if successfully get the pitch value
 * @return false if not find the tf relation
 */
bool AccelBrakeMapCalibrator::getCurrentPitch(double * pitch)
{
  if (get_pitch_method_ == GET_PITCH_METHOD::NONE) {
    *pitch = 0.0;
  } else if (get_pitch_method_ == GET_PITCH_METHOD::IMU) {
    debug_values_.data.at(CURRENT_RAW_PITCH) = imu_pitch_ptr_->data;
    *pitch = lowpass(*pitch, imu_pitch_ptr_->data, 0.2);
    debug_values_.data.at(CURRENT_PITCH) = *pitch;
  } else if (get_pitch_method_ == GET_PITCH_METHOD::TF) {
    geometry_msgs::TransformStamped transform;
    try {
      transform = tf_buffer_.lookupTransform(
        "map", "base_link", ros::Time(0));
    }
    catch (tf2::TransformException & ex) {
      ROS_WARN("%s", ex.what());
      return false;
    }
    double roll, raw_pitch, yaw;
    tf::Quaternion q;
    tf::quaternionMsgToTF(transform.transform.rotation, q);
    tf::Matrix3x3(q).getRPY(roll, raw_pitch, yaw);
    *pitch = lowpass(*pitch, raw_pitch, 0.2);
    debug_values_.data.at(CURRENT_RAW_PITCH) = raw_pitch;
    debug_values_.data.at(CURRENT_PITCH) = *pitch;
  }
  return true;
}

/**
 * @brief check if the data is time out.
 * 
 * @param data_stamp data time to be checked
 * @param current_time current time
 * @param timeout time limit
 * @return true if exceed time limit
 * @return false if in time limit
 */
bool AccelBrakeMapCalibrator::isTimeout(const ros::Time & data_stamp, const ros::Time & current_time, const double timeout)
{
  const double dt = current_time.toSec() - data_stamp.toSec();
  return dt > timeout;
}

/**
 * @brief callback function to get pitch from imu message.
 * 
 * @param msg ros imu message pointer
 */
void AccelBrakeMapCalibrator::callbackImu(const sensor_msgs::Imu::ConstPtr & msg)
{
  auto quaternion = msg->orientation;
  double roll, pitch, yaw;
  tf::Quaternion q;
  quaternion.w = 1.0;
  tf::quaternionMsgToTF(quaternion, q);
  tf::Matrix3x3(q).getRPY(roll, pitch, yaw);
  auto imu_pitch_ptr_temp = std::make_shared<DataStamped>(pitch, msg->header.stamp);
  imu_pitch_ptr_ = imu_pitch_ptr_temp;
}

/**
 * @brief callback function to get brake pedal value and pedal speed
 * 
 * @param msg ros brake report message pointer
 */
void AccelBrakeMapCalibrator::callbackBrake(const pix_hooke_driver_msgs::v2a_brakestafb_531::ConstPtr & msg)
{
  auto brake_pedal_ptr_temp = std::make_shared<DataStamped>(msg->VCU_ChassisBrakePadlFb/100.0, msg->header.stamp);
  // brake_pedal_ptr_->data = msg->Brake_PedalActual;
  // brake_pedal_ptr_->data_time = msg->header.stamp;
  brake_pedal_ptr_ = brake_pedal_ptr_temp;
  if (!brake_pedal_vec_.empty()) {
    const auto past_brake_ptr = getNearestTimeDataFromVec(brake_pedal_ptr_, dif_pedal_time_, brake_pedal_vec_);
    const double raw_brake_pedal_speed = getPedalSpeed(past_brake_ptr, brake_pedal_ptr_, brake_pedal_speed_);
    brake_pedal_speed_ = lowpass(brake_pedal_speed_, raw_brake_pedal_speed, 0.5);
    debug_values_.data.at(CURRENT_RAW_BRAKE_SPEED) = raw_brake_pedal_speed;
    debug_values_.data.at(CURRENT_BRAKE_SPEED) = brake_pedal_speed_;
  }
  debug_values_.data.at(CURRENT_BRAKE_PEDAL) = brake_pedal_ptr_->data;
  pushDataToVec(brake_pedal_ptr_, pedal_vec_max_size_, &brake_pedal_vec_);
  delayed_brake_pedal_ptr_ = getNearestTimeDataFromVec(brake_pedal_ptr_, pedal_to_accel_delay_, brake_pedal_vec_);
}

/**
 * @brief callback function to get throttle pedal value and pedal speed
 * 
 * @param msg ros throttle report message pointer
 */
void AccelBrakeMapCalibrator::callbackThrottle(const pix_hooke_driver_msgs::v2a_drivestafb_530::ConstPtr & msg)
{
  auto accel_pedal_ptr_temp = std::make_shared<DataStamped>(msg->VCU_ChassisThrottlePaldFb/100.0, msg->header.stamp);
  accel_pedal_ptr_ = accel_pedal_ptr_temp;
  if (!accel_pedal_vec_.empty()) {
    const auto past_accel_ptr = getNearestTimeDataFromVec(accel_pedal_ptr_, dif_pedal_time_, accel_pedal_vec_);
    const double raw_accel_pedal_speed = getPedalSpeed(past_accel_ptr, accel_pedal_ptr_, accel_pedal_speed_);
    accel_pedal_speed_ = lowpass(accel_pedal_speed_, raw_accel_pedal_speed, 0.5);
    debug_values_.data.at(CURRENT_RAW_ACCEL_SPEED) = raw_accel_pedal_speed;
    debug_values_.data.at(CURRENT_ACCEL_SPEED) = accel_pedal_speed_;
  }
  debug_values_.data.at(CURRENT_ACCEL_PEDAL) = accel_pedal_ptr_->data;
  pushDataToVec(accel_pedal_ptr_, pedal_vec_max_size_, &accel_pedal_vec_);
  delayed_accel_pedal_ptr_ = getNearestTimeDataFromVec(accel_pedal_ptr_, pedal_to_accel_delay_, accel_pedal_vec_);
}

/**
 * @brief get pedal speed
 * 
 * @param prev_pedal previous pedal data
 * @param current_pedal current pedal data
 * @param prev_pedal_speed previous pedal speed
 * @return double pedal speed
 */
double AccelBrakeMapCalibrator::getPedalSpeed(
  const DataStampedPtr & prev_pedal, const DataStampedPtr & current_pedal, const double prev_pedal_speed)
{
  const double dt = current_pedal->data_time.toSec() - prev_pedal->data_time.toSec();
  if (dt < 1e-3) {
    // invalid pedal info. Return previous pedal speed.
    return prev_pedal_speed;
  }
  const double dp = current_pedal->data - prev_pedal->data;
  return dp / dt;
}

/**
 * @brief callback function for steer
 * 
 * @param msg ros steer report message point 
 */
void AccelBrakeMapCalibrator::callbackSteer(const pix_hooke_driver_msgs::v2a_steerstafb_532::ConstPtr & msg)
{
  auto steer_ptr_tmp = std::make_shared<DataStamped>(msg->VCU_ChassisSteerAngleFb/500*max_steer_, msg->header.stamp);
  steer_ptr_ = steer_ptr_tmp;
  debug_values_.data.at(CURRENT_STEER) = steer_ptr_->data;
}

/**
 * @brief callback function to get velocity and accelleration.
 * 
 * @param msg ros twist message pointer.
 */
void AccelBrakeMapCalibrator::callbackVelocity(const geometry_msgs::TwistStamped::ConstPtr & msg)
{
  geometry_msgs::TwistStamped twist_temp;
  twist_temp.header = msg->header;
  twist_temp.twist.linear.x = std::fabs(msg->twist.linear.x);
  auto twist_ptr_temp = std::make_shared<geometry_msgs::TwistStamped>(twist_temp);
  twist_ptr_ = twist_ptr_temp;

  if (!twist_vec_.empty()) {
    const auto past_msg = getNearestTimeDataFromVec(twist_ptr_, dif_twist_time_, twist_vec_);
    const double raw_acceleration = getAccel(past_msg, twist_ptr_);
    // std::cout<<"xxxxxx"<<raw_acceleration<<std::endl;
    acceleration_ = lowpass(acceleration_, raw_acceleration, 0.25);
    acceleration_time_ = twist_ptr_->header.stamp.toSec();
    debug_values_.data.at(CURRENT_RAW_ACCEL) = raw_acceleration;
    debug_values_.data.at(CURRENT_ACCEL) = acceleration_;

    // calculate jerk 
    if (
      acceleration_time_ - pre_acceleration_time_ > timeout_sec_ || 
      acceleration_time_ - pre_acceleration_time_ <= std::numeric_limits<double>::epsilon()) {
        ROS_WARN_THROTTLE(10, "cannot calculate jerk, time exceeds limit.");
      }
    else { 
      const double raw_jerk = getJerk();
      jerk_ = lowpass(jerk_, raw_jerk, 0.5);
    }
    debug_values_.data.at(CURRENT_JERK) = jerk_;
    pre_acceleration_ = acceleration_;
    pre_acceleration_time_ = acceleration_time_;
  }

  debug_values_.data.at(CURRENT_SPEED) = msg->twist.linear.x;
  pushDataToVec(twist_ptr_, twist_vec_max_size_, &twist_vec_);
}

/**
 * @brief push data to the vector
 * 
 * @tparam T 
 * @param data data to be pushed
 * @param max_size max size of the vector
 * @param vec destination vector
 */
template <class T>
void AccelBrakeMapCalibrator::pushDataToVec(const T data, const std::size_t max_size, std::vector<T> * vec)
{
  vec->emplace_back(data);
  while (vec->size() > max_size) {
    vec->erase(vec->begin());
  }
}

/**
 * @brief get jerk from the acceleration and previous acceleration
 * 
 * @return double jerk in range of [-max_jerk_, max_jerk]
 */
double AccelBrakeMapCalibrator::getJerk()
{
  const double jerk = (acceleration_ - pre_acceleration_) / (acceleration_time_ - pre_acceleration_time_);
  return std::min(std::max(-max_jerk_, jerk), max_jerk_);
}

/**
 * @brief smooth function
 * 
 * @param original original value
 * @param updated new value
 * @param rate rate of sharpness
 * @return double smoothed value
 */
double AccelBrakeMapCalibrator::lowpass(const double original, const double updated, const double rate)
{
  return original * (1.0 - rate) + updated * rate;
}

/**
 * @brief get acceleration from two twist message
 * 
 * @param prev previous twist message
 * @param current current twist message
 * @return double acceleration in range of [min_accel_, max_accel_]
 */
double AccelBrakeMapCalibrator::getAccel(const std::shared_ptr<geometry_msgs::TwistStamped> & prev, const std::shared_ptr<geometry_msgs::TwistStamped> & current)
{
  const double dt = current->header.stamp.toSec() - prev->header.stamp.toSec();
  if (dt < 1e-3) {
    // invalid twist. Return previous acceleration.
    return acceleration_;
  }
  const double dv = current->twist.linear.x - prev->twist.linear.x;
  return std::min(std::max(min_accel_, dv / dt), max_accel_);
}

/**
 * @brief get the data which is nearest to a data with a given backward time from a vector.
 * 
 * @param base_data reference data
 * @param back_time backward time
 * @param vec vector of the data
 * @return DataStampedPtr nearest data that is before the given data with a given time
 */
DataStampedPtr AccelBrakeMapCalibrator::getNearestTimeDataFromVec(
  DataStampedPtr base_data, const double back_time, const std::vector<DataStampedPtr> & vec)
{
  double nearest_time = std::numeric_limits<double>::max();
  const double target_time = base_data->data_time.toSec() - back_time;
  DataStampedPtr nearest_time_data;
  for (const auto & data : vec) {
    const double data_time = data->data_time.toSec();
    const double delta_time = std::abs(data_time - target_time);
    if (nearest_time > delta_time) {
      nearest_time = delta_time;
      nearest_time_data = data;
    }
  }
  return nearest_time_data;
}

/**
 * @brief get the data which is nearest to a data with a given backward time from a vector.
 * 
 * @tparam T ros message pointer with time stamp.
 * @param base_data reference data
 * @param back_time backward time
 * @param vec vector of the data
 * @return T nearest data that is before the given data with a given time
 */
template <class T>
T AccelBrakeMapCalibrator::getNearestTimeDataFromVec(
  const T base_data, const double back_time, const std::vector<T> & vec)
{
  double nearest_time = std::numeric_limits<double>::max();
  const double target_time = base_data->header.stamp.toSec() - back_time;
  T nearest_time_data;
  for (const auto & data : vec) {
    const double data_time = data->header.stamp.toSec();
    const auto delta_time = std::abs(target_time - data_time);
    if (nearest_time > delta_time) {
      nearest_time_data = data;
      nearest_time = delta_time;
    }
  }
  return nearest_time_data;
}


/**
 * @brief add indexs to csv file.
 * 
 * @param csv_file ofstream of the csv file
 */
void AccelBrakeMapCalibrator::addIndexToCSV(std::ofstream * csv_file)
{
  *csv_file << "timestamp,velocity,accel,pitch_comp_accel,final_accel,accel_pedal,brake_pedal,"
            << "accel_pedal_speed,brake_pedal_speed,pitch,steer,jerk,full_original_accel_rmse, "
              "part_original_accel_rmse,new_accel_rmse, rmse_rate"
            << std::endl;
}