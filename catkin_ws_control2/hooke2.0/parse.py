...
with open('dev_kit_pix_hooke2.0_calibration_table.pb.txt', 'r') as f:
    pb_str = f.read()
...
pd_front.to_csv('output/accel_map.csv')
...
pd_back.to_csv('output/brake_map.csv')

with open('output/accel_map.csv', 'r') as f:
    throttle = f.read()
    throttle = 'pix' + throttle

with open('output/brake_map.csv', 'r') as f:
    brake = f.read()
    brake = 'pix' + brake

throttle_file = open('output/accel_map.csv','w')
throttle_file.write(throttle)
brake_file = open('output/brake_map.csv','w')
