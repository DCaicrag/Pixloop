  ...
  const autoware_msgs::VehicleCmdConstPtr vehicle_cmd_ptr)
{
  if (!current_velocity_ptr_ || !acc_map_initialized_) {
    // printf("current_velocity_ptr_:%d \n", current_velocity_ptr_);  !current_velocity_ptr_
    // printf("acc_map_initialized_:%d\n",acc_map_initialized_);!acc_map_initialized_
    // printf("2222");
    return;
  }
  ...
  // ROS_INFO("11111");
  calculateAccelMap(
    *current_velocity_ptr_, vehicle_cmd_ptr->ctrl_cmd.linear_acceleration, &desired_throttle,
    &desired_brake);
  // ROS_INFO("2222");
  ...
  output.control.steering_angle = vehicle_cmd_ptr->ctrl_cmd.steering_angle;
